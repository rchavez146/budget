...
function addCheck(data = {}, options = {}) {
  const { skipCalc = false, showFeedback = true } = options;
  ...
  refreshCheckIndexes();
  if (showFeedback) showToast("Check added");
...
function addAccount(data = {}, options = {}) {
  ...
  refreshBudgetAccountSelectors();
  if (showFeedback) showToast("Account added");
  if (!skipCalc) calculateAll();
}
...
function restoreState() {
  ...
  state.checks?.forEach((check) => addCheck(check, { skipCalc: true, showFeedback: false }));
  state.accounts?.forEach((account) => addAccount(account, { skipCalc: true, showFeedback: false }));
  state.items?.forEach((item) => addItem(item, { skipCalc: true }));

  if (!checksList.children.length) addCheck({}, { skipCalc: true, showFeedback: false });
  if (!accountsList.children.length) {
    addAccount({ type: "Checking" }, { skipCalc: true, showFeedback: false });
    addAccount({ type: "Spending" }, { skipCalc: true, showFeedback: false });
    addAccount({ type: "High-Yield Savings" }, { skipCalc: true, showFeedback: false });
  }
